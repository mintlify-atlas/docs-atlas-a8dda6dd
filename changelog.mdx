---
title: Changelog
description: Complete version history and release notes for gstack
---

All notable changes to gstack are documented here. Each version includes features, fixes, and breaking changes.

<Accordion title="Version 0.3.5 — 2026-03-14">
...
<Accordion title="Version 0.3.4 — 2026-03-13">
...
- **"Just upgraded" confirmation** — after upgrading, the next skill invocation shows "Running gstack v&lbrace;new&rbrace; (just updated!)" via `~/.gstack/just-upgraded-from` marker
...
<Accordion title="Version 0.3.3 — 2026-03-13">
...
<Accordion title="Version 0.3.2 — 2026-03-13">
...
<Warning>
**Breaking Changes:**
- Port scan range 9400-9409 removed
- `CONDUCTOR_PORT` magic offset removed
- Legacy fallback to `~/.claude/skills/gstack/browse/src/server.ts` removed
</Warning>
...
<Accordion title="Version 0.3.1 — 2026-03-12">
...
<Accordion title="Version 0.3.0 — 2026-03-12">
...
<Accordion title="Version 0.0.2 — 2026-03-12">

### Fixed
- Project-local `/browse` installs — compiled binary now resolves `server.ts` from its own directory instead of assuming a global install exists
- `setup` rebuilds stale binaries (not just missing ones) and exits non-zero if the build fails
- `chain` command no longer swallows real errors from write commands (e.g. navigation timeout reported as "Unknown meta command")
- Unbounded restart loop in CLI when server crashes repeatedly on the same command
- Console/network buffers capped at 50k entries (ring buffer) instead of growing without bound
- Disk flush no longer stops silently after buffer hits the 50k cap
...
<Accordion title="Version 0.0.1 — 2026-03-11">
...
<Note>
For migration notes when upgrading between versions with breaking changes, see the [Upgrading](/upgrading) guide.
</Note>